...
# -- Service management -------------------------------------------------------
# Stop services that hold the display/input device.
# pineapplepager must be stopped before we open libpagerctl.so.
# The context manager (with Pager() as p:) in the Python script calls
# pager_cleanup() on exit, which releases the framebuffer cleanly.
# Services are restored in the EXIT trap no matter how we exit.
...
# trap cleanup runs here automatically
