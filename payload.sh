...
# Copy libs into payload's own lib/ so .so is always alongside pagerctl.py
...
# ── Stop pager display service before taking over framebuffer ────────────────
# Must stop LAST and sleep < 1s to beat procd respawn timer
