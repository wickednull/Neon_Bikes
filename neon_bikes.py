...
# Game speed — ms per step. Lower = faster. 80=fast, 120=medium, 160=slow
SPEED = 80
...
def ai_dir(grid, ax, ay, cur_dir):
    ...
    top = candidates[0][0]
    good = [d for s, d in candidates if s >= top * 0.75]
...
class Renderer:
    ...
    def title_screen(self, p1s, p2s):
        ...
        if p1s or p2s:
            p.draw_text_centered(142, f"SCORE   YOU {p1s} : {p2s} CPU", C_WHITE, 1)
...
def run():
    with Pager() as p:
        ...
        while True:
            ...
            # Wait for A (start) or B (quit) — use blocking wait_button()
            while True:
                btn = p.wait_button()
                if btn & Pager.BTN_A:
                    break
            ...
            while not g.over:
                ...
                if pressed & Pager.BTN_UP    and g.p1d != DOWN:  g.p1d = UP
                if pressed & Pager.BTN_DOWN  and g.p1d != UP:    g.p1d = DOWN
                if pressed & Pager.BTN_LEFT  and g.p1d != RIGHT: g.p1d = LEFT
                if pressed & Pager.BTN_RIGHT and g.p1d != LEFT:  g.p1d = RIGHT
            ...
            # ── Game over ────────────────────────────────────────────────────
            if g.result == 'p1':
                p.beep(880, 80); p.delay(90); p.beep(1100, 150)
            elif g.result == 'p2':
                p.beep(300, 200); p.vibrate(300)
            else:
                p.beep(500, 100)
            ...
            while True:
                ...
                if btn & Pager.BTN_A:
                    break        # restart
                if btn & Pager.BTN_B:
                    break        # back to title (scores kept)
