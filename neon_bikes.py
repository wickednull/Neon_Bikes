...
class Renderer:
    ...
    def _heads(self, g):
        if g.p1_alive:
            x = max(0, g.p1x * CELL - 1)
            y = max(0, g.p1y * CELL - 1)
            self.p.fill_rect(x, y, CELL + 2, CELL + 2, C_P1_HEAD)
        if g.p2_alive:
            x = max(0, g.p2x * CELL - 1)
            y = max(0, g.p2y * CELL - 1)
            self.p.fill_rect(x, y, CELL + 2, CELL + 2, C_P2_HEAD)
